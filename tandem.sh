...
bedtools closest -d -k 2 -a $OUT_FILE_PREFIX.query.gff3 -b $2 | awk -v OFS='\t' '{ if ($19 == 0) { if ($13 >= $4) { $19 = -((($5<$14)?$5:$14) - $13 + 1); } else { $19 = -((($5<$14)?5:$14) - $4 + 1); } }; print $0; }' | awk -v OFS='\t' -v min_ovl=$MIN_OVERLAP -v max_dst=$MAX_DISTANCE '{ if (((min_ovl <= -($19))||(min_ovl == 0 && $19 <= max_dst)) && ($4 != $13 || $5 != $14)) { split($9, info1, ";"); split($18, info2, ";"); print $1,$4,$5,$7,$13,$14,$16,substr(info1[1], 4),substr(info2[1], 4),$19; } }' > $OUT_FILE_PREFIX.tmp.tsv
